...
void loop(){                       // Le o valor no potenciometro a cada 1 segundo
  ...
  float valorPot = valorPot*5/1023;
     buffer[2] = (int)valorPot;    // byte menos significativo
     buffer[3] = valorPot - (int)valorPot;       // byte mais significativo
...
void funcaoDadoRecebido(int x){    // funcao chamada ao receber um dado
  ...
  if( dadoRecebido == 0x02 && x == 3 ){  // se for alteracao do limite do alarme
   ...
   if(dadoRecebido == 0x10 && x == 3 ){  // se for alteracao do limite do alarme
    byte dado1 = buffer[0];      // le byte menos significativo
    byte dado2 = buffer[1];      // le byte mais significativo
    int valorRecebido =  dado1 | ( dado2 << 8 );
    if ( valorRecebido >= 0 && valorRecebido <= 1023) {
      buffer[2] = dado1;
      buffer[3] = dado2;
